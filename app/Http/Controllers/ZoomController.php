...
use App\Http\Requests\ZoomScrapeRequest;
use GuzzleHttp;
use Goutte\Client;
use diversen\meta;
...
class ZoomController extends Controller
{
    ...
    public function zoom_scrapper(ZoomScrapeRequest $request)
    {
        ...
        if($validated !== 200) {
            return response()->json([
                'success' => false,
                'message' => "The data could not be scrapped",
                'status' => $validated,
                'jumps' => $count,
                'site' => $this->site_data
            ]);
        ...
        else
        {
            if (isset($this->site_data->url) && isset($this->site_data->hostname) && isset($this->site_data->secured))
            {
                ...
                $this->site_data->contact_email = $this->email_hunter_wrapper($this->site_data->hostname);
                ...
                if (isset($this->site_data->facebook_page)) {
                    $this->site_data->facebook_reach = $this->facebook_stats_wrapper($this->site_data->facebook_page);
                }
                if (isset($this->site_data->twitter_page)) {
                    $this->site_data->twitter_reach = $this->twitter_stats_wrapper($this->site_data->twitter_page);
                }
                ...
                if(isset($this->site_data->facebook_reach))
                    $this->site_data->domain_reach = $this->site_data->domain_reach + intval($this->site_data->facebook_reach/2);

                if(isset($this->site_data->twitter_reach))
                    $this->site_data->domain_reach = $this->site_data->domain_reach + intval($this->site_data->twitter_reach/2);
    ...
    protected function facebook_stats_wrapper($facebook_page)
    {
    ...
    protected function twitter_stats_wrapper($twitter_page)
    {
                ...
                if(count($data) > 1)
                {
                    ...
                    if(!isset($this->site_data->twitter_page))
                        if($data[0] == 'twitter:site')
                        {
                            ...
                            if($this->twitter_stats_wrapper($data[1]) > 0)
                            {
                                $this->site_data->twitter_page = str_replace_first('@', '', $data[1]);
                                $this->site_data->twitter_reach = $this->twitter_stats_wrapper($data[1]);
                            }
                        }
                    if(!isset($this->site_data->facebook_page))
                        if($data[0] == 'article:publisher')
                        {
                            if($this->facebook_stats_wrapper(str_replace_first('/', '', str_replace_first('@', '', explode('facebook.com/', $data[1])[count(explode('facebook.com/', $data[1])) - 1]))))
                            {
                                ...
                                $this->site_data->facebook_reach = $this->facebook_stats_wrapper(str_replace_first('/', '', str_replace_first('@', '', explode('facebook.com/', $data[1])[count(explode('facebook.com/', $data[1])) - 1])));
                            }
                        }
                }
            }
